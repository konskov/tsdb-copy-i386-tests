						 ...
						 register_background_worker_callback_type bgw_register)
{
	...
	while (quit_time > ts_timer_get_current_timestamp() && !ProcDiePending && !ts_shutdown_bgw)
	{
		...
		elog(LOG, "quit_time is %s\n, earliest_wakeup_to_start_next_job is %s", DatumGetCString(DirectFunctionCall1(timestamptz_out, quit_time)), 
		DatumGetCString(DirectFunctionCall1(timestamptz_out, next_wakeup)));
